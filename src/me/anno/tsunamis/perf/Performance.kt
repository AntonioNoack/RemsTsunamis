...
On Tesla P100, 16GB (without 7/8-correction for bandwidth, without correction for two-passes, or for cpu)
...
RX 580 again, more gpu solver variants (without 7/8 correction, without correction for two-passes, or for cpu)
...
fun main(args: Array<String>) {
    ...
    for (type in types) {
        ...
        val bandwidth = when (type) {
            // (3 floats loaded (h,hu,b), 2 floats stored (h,hu)), 2 half-steps, for all cells
            EngineType.CPU -> (3 + 2) * 4 * numIterations * 2 * (width * height).toDouble() / duration / 1e9
