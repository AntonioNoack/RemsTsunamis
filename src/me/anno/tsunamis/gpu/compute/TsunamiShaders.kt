...
val showWavesShader = ShaderLib.createShader(
    "copy", ShaderLib.simplestVertexShader, listOf(Variable("vec2", "uv")), "" +
            ...
            "uniform vec4 mask;\n" +
            "void main(){\n" +
            "   float v = dot(texture(tex, uv), mask);\n" +
...
fun main() {
    ...
    // glMemoryBarrier(GPIXEBUFFER_BARRIER_BIT)

    // glMemoryBarrier(GALBARRIER_BITS)
    /*ImageWriter.writeImageFloat(w + 2, h + 2, "h.png", false, fh)
    ...
    renderVideo(w2, h2, 30.0, desktop.getChild("height.mp4"), numIterations, srcFB) {
        computeTimeStep(timeScale, fluidSim.gravity, src, tmp)
        useFrame(srcFB) {
            src.bind(0)
            val shader = showWavesShader.value
            shader.use()
            shader.v4("mask", 1f, 0f, 0f, 1f)
